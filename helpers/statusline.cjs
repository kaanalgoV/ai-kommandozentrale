...
// Swarm status — reads real agent data from store.json
function getSwarmStatus() {
  // Primary: read actual agent store (ground truth)
  const storePath = path.join(CWD, '.claude-flow', 'agents', 'store.json');
  const storeData = readJSON(storePath);
  if (storeData && storeData.agents) {
    const agents = Object.values(storeData.agents);
    const alive = agents.filter(a => a.status !== 'terminated');
    const active = agents.filter(a => a.status === 'active' || a.status === 'busy');
    return {
      activeAgents: alive.length,
      runningAgents: active.length,
      maxAgents: CONFIG.maxAgents,
      coordinationActive: alive.length > 0,
    };
  }

  // Fallback: metrics files
...
function getSystemMetrics() {
  ...
  // Sub-agents from real agent store (ground truth)
  let subAgents = 0;
  const agentStore = readJSON(path.join(CWD, '.claude-flow', 'agents', 'store.json'));
  if (agentStore && agentStore.agents) {
    subAgents = Object.values(agentStore.agents).filter(a => a.status !== 'terminated').length;
  }
...
function getStdinData() {
  ...
  try {
    ...
    // Read stdin synchronously via file descriptor 0
    const CHUNK = 65536;
    const chunks = [];
    let totalRead = 0;
    while (true) {
      ...
      totalRead += bytesRead;
      if (totalRead > 1024 * 1024) break; // safety cap 1MB
    }
